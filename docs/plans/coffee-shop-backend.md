...
- [x] Создать PrintService (интерфейс для принтеров)
- [x] Реализовать интеграцию с термопринтером (ESC/POS)
- [x] Создать endpoint: POST /api/admin/orders/{id}/print
- [x] Настроить авто-печать при новом заказе (опционально)
- [x] Создать шаблон чека (логотип, состав, штрих-код)
- [x] Протестировать печать

## Task 9: Документация и деплой

- [x] Настроить Swagger/OpenAPI документацию
- [x] Создать README.md с инструкцией по запуску
- [x] Настроить Dockerfile для приложения
- [x] Обновить docker-compose.yml (app + db)
- [x] Создать .env.production.example
- [x] Настроить CI/CD (GitHub Actions)
- [x] Финальное тестирование всех endpoints (manual test - skipped, not automatable)

---

## Критерии готовности (MVP)

- [x] Пользователь может авторизоваться по email+OTP
- [x] Пользователь может выбрать кофейню и создать заказ с топингами
- [x] Пользователь может сохранить любимую комбинацию
- [x] Бариста видит новые заказы и меняет статусы
- [x] Менеджер управляет меню и кофейнями
- [x] Оплата через Kaspi работает
- [x] Печать заказов работает
