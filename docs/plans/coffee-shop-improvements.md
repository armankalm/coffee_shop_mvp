...
**Фронтенд:**
- [x] При каждом изменении `cart` — сериализовать и сохранить в `localStorage` (key: `cart`)
- [x] При старте приложения — прочитать `cart` из `localStorage` и использовать как начальное значение `useState`
- [x] При логауте — очистить `cart` из `localStorage`
- [x] Добавить проверку валидности данных при восстановлении (если формат изменился, сбросить корзину)
